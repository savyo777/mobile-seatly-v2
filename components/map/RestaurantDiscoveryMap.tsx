...
export function RestaurantDiscoveryMap({
  ...
  const focusCluster = useCallback((cluster: MarkerCluster) => {
    ...
    const latitudes = cluster.restaurants.map((restaurant) => restaurant.lat);
    const longitudes = cluster.restaurants.map((restaurant) => restaurant.lng);
    ...
    const nextRegion = {
      ...
      latitudeDelta: Math.max(latSpan * 3, Math.min(currentRegion.latitudeDelta * 0.45, 0.012), 0.003),
      longitudeDelta: Math.max(lngSpan * 3, Math.min(currentRegion.longitudeDelta * 0.45, 0.012), 0.003),
    };

    setCurrentRegion(nextRegion);
    mapRef.current?.animateToRegion(nextRegion, 360);
  }, [currentRegion.latitudeDelta, currentRegion.longitudeDelta, onSelectRestaurant]);
