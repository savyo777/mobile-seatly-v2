...
- Phase K — ToS-coverage centralization (2026-05-21): new `lib/billing/canadianTax.ts` centralizes the province → tax-label mapping (HST/GST/GST+PST/GST+QST) so the checkout breakdown shows the correct provincial tax label per ToS §10.1 commitment instead of a generic "Tax". Used in `app/booking/[restaurantId]/step6-payment.tsx`. The numeric tax rate still comes from `restaurant.taxRate` (server-driven); this util only handles the LABEL. Web team should adopt the same util for parity.
