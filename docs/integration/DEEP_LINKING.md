...
- **Slug:** `restaurants.slug` (unique) — stable for SEO and human-readable URLs on web (e.g. `la-maison` in [Cenaiva seed](https://github.com/StevenGeorgy/Cenaiva/blob/main/supabase/migrations/20250317000013_seed_data.sql)).
...
- **URL scheme:** `cenaiva` — e.g. `cenaiva:///(customer)/discover/<id>`
- **iOS bundle id:** `com.cenaiva.app`
- **Android package:** `com.cenaiva.app`
...
- `expo.ios.associatedDomains` — e.g. `applinks:cenaiva.app`
...
Routes live under [`apps/web/src/routes`](https://github.com/StevenGeorgy/Cenaiva/tree/main/apps/web/src/routes) in the monorepo. Public restaurant URLs should use **slug** in the path for shareability, e.g. `/discover/:slug` or `/r/:slug` — align with your deployed router table.
...
`cenaiva:///(customer)/discover/<id>` for internal QA.
