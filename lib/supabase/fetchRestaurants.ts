...
export async function fetchRestaurantsFromSupabase(): Promise<Restaurant[]> {
  ...
  const explicitPriceRangesById = new Map<string, unknown>(rows.map((row) => [row.id, row.price_range]));
  try {
    return await applyMenuDerivedPriceRanges(supabase, restaurants, explicitPriceRangesById);
  } catch {
    return restaurants;
  }
}
