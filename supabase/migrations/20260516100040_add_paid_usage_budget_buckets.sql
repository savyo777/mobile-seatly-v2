CREATE TABLE IF NOT EXISTS public.paid_usage_buckets (
  bucket_key text PRIMARY KEY,
  scope text NOT NULL CHECK (scope IN ('user_total', 'user_service', 'platform_total', 'platform_service')),
...
ALTER TABLE public.paid_usage_buckets ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.paid_usage_buckets FROM anon, authenticated;

CREATE INDEX IF NOT EXISTS paid_usage_buckets_window_start_idx
  ON public.paid_usage_buckets (window_start);

CREATE INDEX IF NOT EXISTS paid_usage_buckets_user_window_idx
  ON public.paid_usage_buckets (user_key, window_start)
  WHERE user_key IS NOT NULL;

COMMENT ON TABLE public.paid_usage_buckets IS
  'Daily estimated paid API usage buckets for Hey Cenaiva budget enforcement.';
...
CREATE OR REPLACE FUNCTION public.check_paid_usage_budget(
  ...
  p_platform_daily_budget_usd numeric
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_platform_daily_budget_usd IS NULL OR p_platform_daily_budget_usd <= 0 THEN
    RETURN jsonb_build_object('allowed', false, 'reason', 'invalid_platform_budget');
  END IF;
  ...
  v_platform_cap := round(p_platform_daily_budget_usd, 6);
  ...
  PERFORM pg_advisory_xact_lock(hashtext(v_platform_total_key));
  PERFORM pg_advisory_xact_lock(hashtext(v_user_total_key));

  SELECT coalesce(estimated_cost_usd, 0)
  INTO v_platform_spent
  FROM public.paid_usage_buckets
  WHERE bucket_key = v_platform_total_key;
  v_platform_spent := coalesce(v_platform_spent, 0);

  IF v_platform_spent + v_estimated > v_platform_cap THEN
    RETURN jsonb_build_object(
      'allowed', false,
      'reason', 'platform_budget',
      'remaining_user_usd', greatest(v_user_cap, 0),
      'remaining_platform_usd', greatest(v_platform_cap - v_platform_spent, 0)
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'remaining_platform_usd', greatest(v_platform_cap - (v_platform_spent + v_estimated), 0)
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.check_paid_usage_budget(text, text, numeric, numeric, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.check_paid_usage_budget(text, text, numeric, numeric, numeric) TO service_role;
